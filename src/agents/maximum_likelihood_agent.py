...
def maximize(f, bound=[0, 1], res=0.1, tol=1e-4):
    ...
    if res < tol:
        ...
        return maximize(
            f, [max(bound[0], a - res), min(bound[1], a + res)], res / 10, tol
        )


num_activated, num_activated_byme, LL, total_rewards, my_last_action, p_estimate = (
    None,
) * 6
...
def agent(observation, configuration):
    ...
    if observation["lastActions"]:
        ...
        LL[my_last_action] += (
            f" * (p* {d ** (num_activated[my_last_action]-1)})"
            if last_reward
            else f" * (1 - p* {d ** (num_activated[my_last_action]-1)})"
        )
