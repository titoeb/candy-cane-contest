...
class UpperConfidenceBound:
    ...
    def play(self, observation, configuration):
        ...
        t = observation.step
        target_function = self.rewards / self.n_used + self.c * np.sqrt(
            np.log(t + 1) / self.n_used
        )

        best_action = np.argmax(target_function)
        best_action_mask = target_function == target_function[best_action]

        if best_action_mask.sum() > 1:
            return int(np.random.choice(np.where(best_action_mask)[0]))
        else:
            best_action
