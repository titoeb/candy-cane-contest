...
class UCBML:
    def __init__(self, n_bins: int, exploration: float, model_str: str):
    ...
    def play(self, observation, configuration):
        ...
        success_ratio = self.predictions
        exploration = self.exploration * np.sqrt(np.log(current_step + 1) / (self.n_pulls + 1e-10))

        return int(np.argmax(success_ratio + exploration))

ucb_ml = UCBML(n_bins=100, exploration=0.01, model_str=model)

def agent(observation, configuration):
    return ucb_ml.play(observation, configuration)
